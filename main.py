...
#Decalring base variables and constants

st.header("UBC Course Compare")
st.sidebar.header("Enter Course details for Comparison")
st.sidebar.caption('Compare courses from 2014 to 2021')
...
cc1 = st.selectbox(
    "Enter Course Code for Course 1 (for eg: MATH):",
    course_list, 71    )

year1 = st.sidebar.selectbox(
     'Enter the year for the Course 1:',
...
#Filtering Data for Course 1
url = url + '/' + str(year1) + str(term1) +"/"
url = url + "UBCV-" + str(year1) + str(term1) +"-" + cc1 +".csv"
course_num1 = pd.read_csv(url)
course_number_set1 = sorted(set(course_num1["Course"]))

cn1 = st.selectbox(
    "Enter Course Number for Course 1(for eg: 221, 121 etc)", course_number_set1)

#Loading DataFrame 1

course_num1 = pd.read_csv(url)
load_data1 = course_num1[(course_num1['Course'] == int(cn1))]
load_data3 = load_data1
load_data3 = load_data3.drop(['Detail', "Title", "Section", "Enrolled", "Campus", "Session", "Professor"], axis =1)
a= load_data1.Title[load_data1["Section"] == "OVERALL"]
st.write(str(a[a.index].values))
st.write(load_data3)
course_number_set1 = set(course_num1["Course"])

#Loading Visualisations
st.write("Averages by Professor")

viz1 = pd.DataFrame({'professors': load_data1.Professor,
                       'avg': load_data1.Avg,
                         'high':load_data1.High,
                         'low':load_data1.Low,
                         'section':load_data1.Section})
viz1

prof_chart1 = pd.DataFrame (load_data1.Avg.values, load_data1.Professor.values)

st.bar_chart(prof_chart1)
...
cc2 = st.selectbox("Course Code for Course 2",course_list)

y2 = st.sidebar.selectbox(
     'Enter the year for the Course 2.',
     (list(range(2014, 2022))), 7)

t2 = st.sidebar.radio(
     "Select Term for Course 2:  Winter (W), Winter (S)",
     ('W', 'S'))


#Filtering Data for Course 2
url2 = url2 + '/' + str(y2) + str(t2) +"/"
url2 = url2 + "UBCV-" + str(y2) + str(t2) +"-" + cc2 +".csv"
course_num2 = pd.read_csv(url2)
course_number_set2 = set(course_num2["Course"])

cn2 = st.selectbox(
    "Enter Course Number for Course 2 (for eg: 221, 121 etc)", course_number_set2)
...
load_data4 = load_data2
load_data4 = load_data4.drop(['Detail', "Title", "Section", "Enrolled", "Campus", "Session", "Professor"], axis =1)
b = load_data2.Title[load_data2["Section"] == "OVERALL"]
st.write(str(b[b.index].values))
st.write(load_data4)
course_number_set2 = set(course_num2["Course"])

#Visualising the result
st.write("Averages by Professor")
viz2 = pd.DataFrame({'professors': load_data2.Professor,
                       'avg': load_data2.Avg,
                         'high':load_data2.High,
                         'low':load_data2.Low,
                         'section':load_data2.Section})
viz2

prof_chart2 = pd.DataFrame (load_data2.Avg.values, load_data2.Professor.values)

st.bar_chart(prof_chart2)
...
# url1 = """https://raw.githubusercontent.com/DonneyF/ubc-pair-grade-data/1516765eb6fd962066149b18ec8c6d64ae06046a/tableau-dashboard/UBCV/{}/{}""".format(comb1,comb2,)
#
# load_data1 = course_num1[(course_num1['Course'] == int(cn1))]
